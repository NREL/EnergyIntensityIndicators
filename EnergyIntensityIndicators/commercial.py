...
class CommercialIndicators(LMDI):
    ...
    def __init__(self, energy_data, activity_data, categories_list):
        ...
        self.conversion_factors = self.eia_comm.conversion_factors()
        # self.cbecs = 
        ...
        self.SEDS_CensusRgn = self.eia_comm.get_seds()
        self.national_calibration = self.eia_comm.national_calibration()
        ...
        self.mer_data_23 = self.eia_comm.eia_api(id_='711251', id_type='category')
    ...
    def adjusted_supplier_data(self):
        ...
        print(type(published_consumption_trillion_btu))
        print(published_consumption_trillion_btu)
        # 1970-2018
        published_consumption_trillion_btu.append(list(self.national_calibration['Final Est. (Trillion Btu)_elec']))  # Column G (electricity final est) # for years 1970-2018
        print(published_consumption_trillion_btu)
        # 1977-1989
    ...
    def fuel_electricity_consumption():
        """Trillion Btu
        """
        total_primary_energy_consumption = self.eia_api(id_='TOTAL.TXCCBUS.A', id_type='series') # pre 1969: AER table 2.1c update column U 
        total_primary_energy_consumption = total_primary_energy_consumption[total_primary_energy_consumption['year'].isin(range(1949, 1970))]
        total_primary_energy_consumption = total_primary_energy_consumption.multiply(0.001)
        fuels_dataframe = pd.concat([total_primary_energy_consumption, self.national_calibration]) # >= 1970: National Calibration Column 0
